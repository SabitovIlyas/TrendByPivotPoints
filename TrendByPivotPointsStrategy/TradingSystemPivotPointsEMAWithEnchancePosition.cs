...
{
    public class TradingSystemPivotPointsEMAWithEnchancePosition
    {
        ...
        bool enchancePosition = false;

        public TradingSystemPivotPointsEMAWithEnchancePosition(LocalMoneyManager localMoneyManager, Account account, Security security, PositionSide positionSide)
        {   
        ...
        public void CheckPositionOpenLongCase(double lastPrice, int barNumber)
        {
            ...
            else
            {
                ...
                var closeCaseAccepted = CheckLongPositionCloseCase(le, barNumber);

                if (enchancePosition)
                {
                    if (!closeCaseAccepted)
                    {
                        #region Наращивание позиции

                        Logger.Log("Проверяем условия наращивания позиции: выполняется ли условие двух последовательных повышающихся минимумов?");
                        ...
                        if (patternPivotPoints_1g2.Check(lowsValues))   //1
                        {
                            ...
                            {
                                Logger.Log("Не найден последний минимум, использованный при открытии длинной позиции");
                                UpdateStopLossLongPosition(barNumber, lows, lastLow, le);
                            }
                        }
                        else
        ...
        public void CheckPositionOpenShortCase(double lastPrice, int barNumber)
        {
            ...
            else
            {
                ...
                var closeCaseAccepted = CheckShortPositionCloseCase(se, barNumber);
                ...
                if (enchancePosition)
                {
                    if (!closeCaseAccepted)
                    {
                        ...
                        Logger.Log("Проверяем условия наращивания позиции: выполняется ли условие двух последовательных понижающихся максимумов?");
                        ...
                        if (patternPivotPoints_1l2.Check(highsValues))   //1
                        {
                            ...
                            if (lastHighForOpenShortPosition != null)
                            {
                            ...
                            {
                                Logger.Log("Не найден последний максимум, использованный при открытии короткой позиции");
                                UpdateStopLossShortPosition(barNumber, highs, lastHigh, se);
                            }
                        }
                        else
